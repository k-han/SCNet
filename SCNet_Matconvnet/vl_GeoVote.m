function [ confidenceMap, tableBina] = vl_GeoVote(viewA, viewB, A)
...
width = viewB.imgSz(2)*2;
height = viewB.imgSz(1)*2;
nCell = 8000;  % cells in x-y space
szCell = max(sqrt((width*height)/nCell),5);

model.nSpatialX = round(width/szCell*zoom_xy);
model.nSpatialY = round(height/szCell*zoom_xy);
model.nSpatialS = round(20*zoom_s);
...
confidenceMap(idxValid) = houghspace(tableBina(idxValid));
end
