...
function dense_flow_eva_ft_PASCAL()
...
for ft = 1:numel(conf.feature)
    ...
    for ci = 1:length(conf.class)
        ...
        fileID = fopen(fullfile(conf.evaDFDir,[ conf.class{ci} '-' func2str(conf.proposal) '-' conf.feature{ft} '_ft.txt']),'w');
        fprintf(fileID,'%s\n', conf.class{ci});
        ...
        save(fullfile(conf.evaDFDir, conf.class{ci},...
            [ conf.class{ci} '_' func2str(conf.proposal) '_' conf.feature{ft} '_ft.mat' ]), 'PCK');
    ...
    fileID = fopen(fullfile(conf.evaDFavgDir,[ func2str(conf.proposal) '-' conf.feature{ft} '_ft.txt']),'w');
    ...
    save(fullfile(conf.evaDFavgDir, [ func2str(conf.proposal) '_' conf.feature{ft} '_ft.mat' ]), 'avg_PCK');
end
