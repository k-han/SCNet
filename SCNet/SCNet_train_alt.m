function [net,stats] = SCNet_train_alt(net, imdb, getBatch, varargin)
...
global loss_type
 loss_type = 'dagnn.PFLoss';
% loss_type = 'dagnn.ProposalDotL2'; 
% loss_type = 'dagnn.ProposalSiamContrast';
...
if isempty(opts.train), opts.train = find(imdb.data.set==1) ; end
if isempty(opts.val), opts.val = find(imdb.data.set==2) ; end
...
%     if fix((t-1)/opts.batchSize)+1 == 27
%         fprintf('debug here');
%     end
  ...
  for s=1:opts.numSubBatches
    ...
    if strcmp(mode, 'train')
      ...
      % alternative fixing the params
        ...
        [net.params([mIdx_1f mIdx_1b]).learningRate] = deal(mod(state.epoch,2));
        [net.params([mIdx_2f mIdx_2b]).learningRate] = deal(mod(state.epoch + 1,2));
...
% -------------------------------------------------------------------------
global loss_type
sel = find(cellfun(@(x) isa(x,loss_type), {net.layers.block})) ;
stats = struct() ;
for i = 1:numel(sel)
