...
for lIdx=1:NL
    ...
    if (isa(net.layers(lIdx).block, 'dagnn.Conv') && opts.plotTexts==1)
        text(Lxy(lIdx,1)-hshift, Lxy(lIdx,2),  ['p' num2str(net.layers(lIdx).paramIndexes(1)) ',p' num2str(net.layers(lIdx).paramIndexes(2)) ': ' size2string(net.layers(lIdx).block.size)], 'VerticalAlignment','middle','HorizontalAlignment','center','FontSize',round(opts.textSize*2/3));
        text(Lxy(lIdx,1)-hshift, Lxy(lIdx,2)-0.4,  [' stride: ' size2string(net.layers(lIdx).block.stride)], 'VerticalAlignment','middle','HorizontalAlignment','center','FontSize',round(opts.textSize*2/3));
    end
end
